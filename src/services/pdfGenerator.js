...
function generatePDF(departureType, departureData, filePath) {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(18).font('Helvetica-Bold').text('PARROQUIA DE LABRANZAGRANDE BOYACÁ', 130, 60, { width: 400, align: 'center' });
...
function generateConfirmationContent(doc, data) {
  doc.text(`En el municipio de Labranzagrande (Boyacá), a los ${data.confirmationDate.getDate()} días del mes de ${getMonthName(data.confirmationDate.getMonth())} del año ${data.confirmationDate.getFullYear()}, el Excelentísimo Párroco Victor Cardenas,`, {
    lineGap: 5
  });
  doc.moveDown(2);
...
function generateMarriageContent(doc, data) {
  doc.text(`En el municipio de Labranzagrande (Boyacá), a los ${data.marriageDate.getDate()} días del mes de ${getMonthName(data.marriageDate.getMonth())} del año ${data.marriageDate.getFullYear()}. Ante mí, Victor Cardenas, Párroco de esta Parroquia,`, {
    lineGap: 5
  });
  doc.moveDown(2);
...
function generateDeathContent(doc, data) {
  doc.text(`En el municipio de Labranzagrande (Boyacá), a los ${data.deathDate.getDate()} días del mes de ${getMonthName(data.deathDate.getMonth())} del año ${data.deathDate.getFullYear()}. Yo, el infrascrito Victor Cardenas, Párroco de esta Parroquia,`, {
    lineGap: 5
  });
